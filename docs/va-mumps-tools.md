...
Each status is a **two-circle pair**. The **first (primary) circle is the verdict** — 🟢 **Authorized** or 🔴 **Unauthorized** — and the **second (secondary) circle is the specific TRM condition**, drawn in that condition's VA TRM color. An arrow (e.g. **🟢🟡→🔴🟠**) marks an entry that transitions across the TRM matrix timeline — the **right-hand pair is the current/effective decision** (as of ~May 2026, i.e. past the Q1 2026 boundary); the left pair is the prior state detailed in §5.

- **Primary circle — verdict:** 🟢 Authorized (usable, subject to the secondary condition) · 🔴 Unauthorized (not usable unless the secondary condition is satisfied). A bare **—** means no TRM entry exists (not assessed).
- **Secondary circle — condition (VA TRM color):** 🟢 none · 🟡 with constraints · ⚪ POA&M review **[a]** · 🟠 divest **[b]** · 🔵 under evaluation **[c]** · ⚫ unconditional (flatly prohibited).

*(Per your earlier request, 🟢 green replaces the TRM's white for "Authorized"; GitHub-flavored Markdown has no gray circle, so ⚪ stands in for the TRM's gray.)*

| Glyphs | TRM color | Decision | Definition |
|:---:|---|---|---|
| 🟢🟢 | White → **green** | **Authorized** | The technology/standard has been authorized for use. |
