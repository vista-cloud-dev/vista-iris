...
Glyphs use the **VA TRM decision-matrix color scheme**, with one change you requested: 🟢 **green** denotes *Authorized* in place of the TRM's white. An arrow (e.g. **🟡→🟠**) marks an entry that transitions across the TRM matrix timeline — the **right-hand glyph is the current/effective decision** (as of ~May 2026, i.e. past the Q1 2026 boundary); the left glyph is the prior state detailed in §5. *(GitHub-flavored Markdown has no gray circle, so ⚪ stands in for the TRM's gray.)*

| Glyph | TRM color | Decision | Definition |
|:---:|---|---|---|
...
> **Mapping note.** The glyph per row reflects this brief's mapping of each entry's captured TRM decision language to the six-color scheme: the **Caché family** (Caché, ObjectScript, Studio, CSP, Objects, ODBC Driver, Management Portal, Zen) and **Ensemble** are explicitly **Divest** 🟠; **FIS-GT.M**, **MUMPS (ANSI edition)**, and **Atelier** carry the POA&M-only **gray [a]** ⚪ flavor. ⚫ and 🔵 are defined for completeness but are not currently assigned to any entry here. **Re-verify on the live TRM Decision tab before relying on any status.**
