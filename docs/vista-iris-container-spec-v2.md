...
**Status:** Draft spec · **Version:** 2 · **Date:** 2026-05-23 · **Supersedes:** `vista-iris-container-spec.md` (v1) · **Scope:** A reproducible, lightweight, cross-platform (Linux + macOS, x86-64 + Apple Silicon) container that stands up a *functioning* VistA EHR instance — reachable by a CPRS client over **RPC** and by external systems over **HL7** — with sample patients, on InterSystems IRIS **Community Edition**, plus the inner-loop developer tooling a VA VistA developer would actually use.
...
> 4. **§4 Base Image** — keeps tag-pinning; now **recommends `irishealth-community`** because HL7/FHIR are in scope.
> 5. **§5 VistA Source** — adds a **source-level quality audit** of the OSEHRA scripts, a **catalog of the installer's options**, and a **bare-bones streamlining** decision (drop EWD.js, GT.M/YottaDB, dashboards, etc.).
...
6. Exposes documented endpoints for verification (terminal/roll-and-scroll, Management Portal, RPC Broker, HL7 MLLP).
...
- **Reproducible:** image build is deterministic from pinned source tags; the build and the CI/CD chain are both expressed as `make` targets.
...
- **No EWD.js / web-framework layer, no GT.M/YottaDB, no CDash dashboards** — see the streamlining decision in §5.3.

---

## 3. Container Technology Decision

**Recommendation (changed in v2): Podman as the primary runtime; Docker remains drop-in compatible.**
...
**Pin the tag** (unchanged from v1) **and prefer `intersystems/irishealth-community`** (changed in v2).

Because HL7/FHIR interfacing is now an explicit goal (§1, items 4 & 6; §10), the base image should be the **healthcare edition**, which bundles the HL7 v2 / FHIR interoperability engine (interoperability productions, MLLP business services/operations, and a FHIR server) on top of the same M engine:

| Choice | When |
|---|---|
| **`intersystems/irishealth-community`** ✅ recommended | HL7/FHIR in scope (this spec). Adds the interoperability + FHIR engine; VistA itself runs identically (M routines/globals are unchanged across IRIS editions). FHIR REST is served on the web port; HL7 MLLP via interop productions. |
| `intersystems/iris-community` | Acceptable if you only need VistA's *own* HL7 (HL package over MLLP) and the RPC Broker, and do not want IRIS's FHIR server. VistA + RPC + native HL7 still work; you forgo IRIS's FHIR endpoint. |
...
- **Architectures:** `amd64` and `arm64` (append `-arm64` to the tag) → covers Linux + Apple Silicon Macs.
...
> **Pin the tag** (e.g. a specific `irishealth-community:2025.x`) in the spec'd build for reproducibility; `latest-cd` is convenient but drifts. The pinned tag is a `Makefile`/build argument so the arch-specific variant is selectable without editing the `Dockerfile`.

---

## 5. VistA Source Decision

**Agreed sources (unchanged):**
...
The OSEHRA "installation script" with command-line options is `Scripts/Install/<Ubuntu|RHEL>/autoInstaller.sh` (the Ubuntu and RHEL variants share an identical option set). It is **GT.M/YottaDB-centric**; its Caché branch is a stub. Full option list, with the keep/drop decision for a bare-bones VA-developer IRIS instance:
...
| `-c <path>` | Path to **Caché** installer | ❌ drop | We use the official **IRIS** community **image**, not a Caché installer. (And this branch is a stub anyway — §5.2.) |
| `-d` | Create GT.M/YottaDB **development directories** (`s` & `p`) | ❌ drop | GT.M/YottaDB-only concept (filesystem routine/object dirs). On IRIS, routines live in the namespace. |
| `-e` | Install **EWD.js** (Node.js web framework) | ❌ **drop** | Not part of core VistA; not a VA-used tool. See §5.2/§5.3. |
| `-g` | Use **GT.M** | ❌ drop | We target IRIS. |
| `-y` | Use **YottaDB** | ❌ drop | We target IRIS. |
...
- `Scripts/Install/GTM/install.sh` — options `-h`, `-s` (skip), `-v <ver>`, `-y` (YottaDB). **N/A** (GT.M/YottaDB only).
- `Scripts/Install/GTM/createVistaInstance.sh` — options `-h`, `-f` (skip firewall), `-i <name>`, `-y`. **N/A** (GT.M/YottaDB only).
- `Scripts/Install/EWD/ewdjs.sh` — options `-h`, `-v <node-ver>`. **Dropped** (see below).

### 5.2 Source-level quality findings

Comprehensive review of the candidate scripts surfaced the following. These justify *reusing the Python/CMake import path and not the legacy shell provisioners*:
...
**Do not reuse (obsolete / out-of-scope / risky):**
...
2. **`autoInstaller.sh` has a dead/broken guard.** The "no M environment viable" check (`if [[ ! $installgtm || ! $cacheinstallerpath || ! $installYottaDB ]]`) negates non-empty strings (`"false"`), so it never triggers — and the logic should be conjunctive, not disjunctive. The intended safety check does nothing.
...
**Include (bare bones):**
- IRIS (Health) Community + the **FOIA VistA routines & globals** (pinned).
...
│  FROM intersystems/irishealth-community:<pinned>                       │
│                                                                        │
│   ┌────────────────────────────────────────────────────────────────┐ │
...
**Strategy A is recommended** (unchanged) and is made **ephemeral and fully portable**: everything required to build, run, verify, and tear down the instance lives in a **`Dockerfile` + `Makefile`** committed to the repo — no host-specific setup, no manual steps.
...
1. `FROM intersystems/irishealth-community:<pinned>`
2. `COPY` the pinned VistA-M sources + the (streamlined, §5.3) ObjectScript/`iris.script`/Python install orchestration into the image.
3. Start IRIS quietly → run the install orchestration as an IRIS session/`irissession` flow to: create DB/namespace, set mappings, import routines, load globals, initialize the OS interface, configure TaskMan, the **RPC Broker listener**, and the **HL7 Link Manager**, then load Tier-1 sample data.
...
### 7.1 Makefile-driven build and CI/CD chain (new in v2)
...
| `make build` | Build the OCI image (Strategy A) via `$(ENGINE)` (defaults to `podman`, override `ENGINE=docker`), passing the pinned base-image tag + arch as build args. |
...
Executed as ObjectScript/M (and the OSEHRA Python step scripts), in the order the audited `ImportRG.cmake` + `PostImportSetupScript` enforce, adapted to IRIS:
...
6. **Post-install configuration** (OSEHRA `PostImportSetupScript`)
...
Must:
- Base on a **pinned** `intersystems/irishealth-community` tag, exposed as a build argument so the architecture-specific variant (append `-arm64` on Apple Silicon) is selectable without editing the file.
- Copy the pinned VistA-M sources and the **streamlined** (§5.3) install orchestration into a working directory owned by the `irisowner` account — no EWD.js, no GT.M/YottaDB, no dashboard tooling, no run-time fetches from personal repos.
...
### 11.3 Build & CI/CD definition (`Makefile`) — new in v2
...
### 11.4 Install orchestration (`scripts/`)
A top-level orchestration entry point (invoked at build under Strategy A) that performs §8 in order by delegating to per-step scripts (§12), reusing the OSEHRA Python/CMake import path parameterized for IRIS. Requirements:
...
| **IRIS vs Caché compatibility** | OSEHRA path targets Caché; expect tweaks to `%Z*`/`^%ZOSF` OS layer, locale/charset, a few `$ZU` calls, and `irissession` vs `iris session`. **Highest-risk item — validate early with a minimal import.** |
...
| **HL7 / FHIR engine** | `irishealth-community` carries the interop/FHIR engine (chosen in §4). VistA's *own* HL7 (HL package over MLLP) works on either edition; the FHIR REST front door requires the Health edition. Confirm Community-edition interop limits are adequate for dev. |
...
| **Legacy OSEHRA shell installers not reused** | `Cache/install.sh` (Caché 2011), `autoInstaller.sh` (GT.M-centric, broken guard), `ewdjs.sh` (Node 0.12) are obsolete/out-of-scope (§5.2). Risk = drift if someone reaches for them; mitigate by reusing only the Python/CMake import path. |
| **Image size vs boot time** | Strategy A (baked) trades a bigger image for instant boot — preferred for demos/ephemeral use. |
| **No real PHI** | All patients fictitious; banner the instance as test-only. EWD's "disable Access/Verify encryption" hack is **excluded** (§5.2/§5.3). |
...
2. **Build and start** — `make build && make up` (Podman by default; `ENGINE=docker make ...` to use Docker) produces and runs the loaded instance.
...
- InterSystems IRIS for Health Community Edition — [Docker Hub `intersystems/irishealth-community`](https://hub.docker.com/r/intersystems/irishealth-community) · IRIS Community — [`intersystems/iris-community`](https://hub.docker.com/r/intersystems/iris-community)
...
- OSEHRA VistA build/install automation — [github.com/WorldVistA/VistA](https://github.com/WorldVistA/VistA): IRIS-aware import/config path (`Testing/Setup/ImportRG.cmake`, `Testing/Setup/{Initialize,RoutineImport,GlobalImport,PostImportSetupScript,ClinicSetup}.py.in`, `Python/vista/{OSEHRAHelper,OSEHRASetup}.py`); option-bearing installers audited (`Scripts/Install/{Ubuntu,RHEL}/autoInstaller.sh`, `Scripts/Install/Cache/install.sh`, `Scripts/Install/EWD/ewdjs.sh`); import docs (`Documentation/{InstallCache,ImportCache,ConfigureCache,AutomatedVistAConfiguration}.rst`).
