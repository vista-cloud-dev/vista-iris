...
Each status is a **two-circle pair**: the **first circle is the verdict**, the **second circle is the condition**. An arrow (e.g. **⚪🔘→⚫🔴**) marks an entry transitioning across the TRM timeline — the **right-hand pair is the current/effective decision** (as of ~May 2026, past the Q1 2026 boundary); the left pair is the prior state detailed in §5.

- **Verdict (first circle):** ⚪ **Authorized** · ⚫ **Unauthorized** · **—** no TRM entry (not assessed).
- **Condition (second circle):** for **Authorized** — ⚪ none (fully authorized) / 🔘 *gray* = **with constraints**; for **Unauthorized** — ⚫ none (flat, unconditional prohibition) / one of **three "Conditions Required" colors** below, ordered by **severity**:

| Severity | Glyphs | Condition `[id]` | One word | Why it ranks here |
|:---:|:---:|---|---|---|
...
> **Gray-glyph note.** GitHub-flavored Markdown has no gray-circle emoji, so 🔘 (radio button) stands in for the TRM's **gray** "with constraints" color; ⚪/⚫ are the literal white/black verdict circles.

| Glyphs | TRM color | Decision | Definition |
|:---:|---|---|---|
| ⚪⚪ | White + white | **Authorized** | The technology/standard has been authorized for use. |
| ⚪🔘 | White + **gray** | **Authorized w/ Constraints** | The technology/standard can be used within the specified constraints located below the decision matrix in the footnote [1] and on the General tab. |
| ⚫⚫ | Black + black | **Unauthorized** | The technology/standard is not (currently) permitted to be used under any circumstances. |
| ⚫🟡 | Black + **yellow** | **Unauthorized, Conditions Required `[a]` — POA&M review (Remediate)** | This technology or standard can be used only if a POA&M review is conducted and signed by the Authorizing Official Designated Representative (AODR) as designated by the Authorizing Official (AO) or designee and, based upon a recommendation from the POA&M Compliance Enforcement, has been granted to the project team or organization that wishes to use the technology. *(Least severe — a defined, achievable path to use.)* |
| ⚫🟠 | Black + **orange** | **Unauthorized, Conditions Required `[c]` — Under evaluation (Piloting)** | The period of time this technology is currently being evaluated, reviewed, and tested in controlled environments. Use of this technology is strictly controlled and not available for use within the general population. If a customer would like to use this technology, please work with your local or Regional OI&T office and contact the appropriate evaluation office displayed in the notes below the decision matrix. *(Medium severity.)* |
| ⚫🔴 | Black + **red** | **Unauthorized, Conditions Required `[b]` — Divest (Retiring)** | VA has decided to divest itself of the use of the technology/standard. As a result, all projects currently utilizing the technology/standard must plan to eliminate their use of the technology/standard. Additional information on when the entry is projected to become unauthorized may be found on the Decision tab for the specific entry. *(Most severe — terminal, no forward runway.)* |

> **Mapping note.** The glyph pair per row reflects this brief's mapping of each entry's captured TRM decision language to the six-status scheme: the **Caché family** (Caché, ObjectScript, Studio, CSP, Objects, ODBC Driver, Management Portal, Zen) and **Ensemble** are explicitly **Divest** ⚫🔴; **FIS-GT.M**, **MUMPS (ANSI edition)**, and **Atelier** carry the POA&M-only **yellow `[a]`** ⚫🟡 flavor; the authorized-with-constraints entries (IRIS for Health, the VS Code Extension Pack, HealthShare, IAM) and the **ISO** edition of MUMPS are ⚪-primary (⚪🔘). **⚫⚫** (flat prohibition) and **⚫🟠** (under evaluation) are defined for completeness but are not currently assigned to any entry here; **YottaDB** has no TRM entry (shown as **—**). **Re-verify on the live TRM Decision tab before relying on any status.**
